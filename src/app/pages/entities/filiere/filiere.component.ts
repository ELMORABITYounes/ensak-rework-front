...
export class FiliereComponent implements OnInit, OnDestroy {
  ...
  constructor(
        ...
        this.filiereService
            ...
            .subscribe(
                (res: HttpResponse<IFiliere[]>) => {
                  this.loading=false;
                  console.log(res);
                  this.paginateFilieres(res.body, res.headers)
                },
    ...
    trackId(index: number, item: IFiliere) {
        return item.id;
    }

    registerChangeInFilieres() {
        this.eventSubscriber = this.eventManager.subscribe('filiereListModification', response => this.loadAll());
    }
    ...
    protected paginateFilieres(data: IFiliere[], headers: HttpHeaders) {
